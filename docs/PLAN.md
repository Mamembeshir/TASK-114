...
- [x] Audit all pages for visual consistency (spacing, typography, colors)
...
| 3   | High       | Anti-sniping "once per auction" divergence                                        | ✅ Resolved — CLAUDE.md §4 states "Extend by 2 minutes every time a bid is placed in the final 30-second window. Each snipe bid resets the clock; no cap on extensions." Engine implements unlimited extensions with no gate. `antiSnipingTriggered` / `antiSnipingTriggeredAt` fields are record-keeping markers (first extension timestamp), not enforcement flags. Bid timeline shows the extension event row. |
...
| 2   | High     | Anti-sniping extends once per auction; prompt implies repeated extension           | ✅ Resolved — implementation allows **unlimited extensions** per CLAUDE.md §4: "Extend by 2 minutes every time a bid is placed in the final 30-second window. Each snipe bid resets the clock; no cap on extensions." `biddingEngine.ts` applies the extension on every qualifying bid with no gate. `antiSnipingTriggered` / `antiSnipingTriggeredAt` are record-keeping markers, not limiters. |
...
## Third Audit Finding Resolutions

| #   | Severity | Finding                                                                            | Resolution                                                                                                                                                                                                                                                                                                                                                              |
| --- | -------- | ---------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Blocker  | Offline-only requirement violated by remote URL-based assets and attachments       | ✅ **Service-level validation added.** All create/update service methods for auctions, catalog items, documents, and publications now reject external `http(s)` URLs via `isExternalUrl()`. Form pages use `isExternalUrl()` consistently (replaced `startsWith('http')` checks). `sanitizeHtml` already strips `<img>` tags. `decodeLocalFile` already returns null for external URLs. |
| 2   | High     | Auction increment defaults diverge from prompt ($1 under $50, $5 $50–$500, $10 above $500) | ✅ **Defaults corrected.** `DEFAULT_INCREMENT_TIERS` updated to `{upTo:50,increment:1}, {upTo:500,increment:5}, {upTo:null,increment:10}`. `defaultMinimumIncrement` system config default changed from `100` to `1` in `SystemSettingsPage` and `documentService` singleton initializer. |
| 3   | High     | Bid UI uses flat increment math, disagrees with tier-aware bidding engine           | ✅ **BidForm and AuctionDetailPage now use `getMinimumIncrement()`** — the same tier-aware helper used by the engine — to compute display min bid, input step, and hint text. Flat `auction.minimumIncrement` references replaced with resolved increment. |
| 4   | High     | Documentation materially inconsistent with actual startup/auth behavior             | ✅ **Fixed.** `CLAUDE.md` `questions.md` path corrected to `../docs/questions.md`. Stale seed-account password comment in `authStore.ts` updated to match actual values. README already correct (pnpm, dev-only seeding, Bootstrap Admin wizard). |
| 5   | High     | Participant role can access controlled document module contrary to prompt role scope | ✅ **Removed `viewDocuments` and `checkoutDocument` from `Role.Participant`** in `permissions.ts`. Document routes already guarded by `viewDocuments`. Nav drawer already hides Documents when permission absent. RBAC test updated to assert denial. |
| 6   | High     | Document reads not object/status-scoped; broad access to all docs via list/detail  | ✅ **Object/status scoping added** to `listDocuments` and `getDocumentById`. Admin sees all; Reviewer sees own + InReview/Approved/PendingDestruction/Archived; ContentEditor sees own + Approved/Archived. `isDocumentVisible()` enforces this at the service layer. |
| 7   | High     | Notification object-level auth trusts caller-supplied user IDs                     | ✅ **`markNotificationRead`, `deleteNotification`, `recordReadReceipt` now derive actor from `useAuthStore`** instead of accepting a caller-supplied userId parameter. All callers updated. Tests rewritten to set auth state before each call. |
| 8   | High     | Core notification creation/queue primitives lack function-level auth guards        | ✅ **`requireAuthenticated()` guard added** to `createNotification`, `createNotificationForMany`, and `queueOutboundMessage`. These primitives now reject calls from unauthenticated contexts. Tests updated to set auth state in `beforeEach`. |
| 9   | Medium   | Bid timeline does not fully trace repeated anti-sniping events                     | ✅ **First-class `AuctionExtensionEvent` records** persisted in new `auctionExtensionEvents` IndexedDB table (DB v8). Engine writes an event per extension with `triggeringBidId`, previous/new end times. Timeline renders all extension rows per bid instead of a single synthetic row from `antiSnipingTriggeredAt`. |
| 10  | Medium   | Admin data export is JSON-only; CSV parity not implemented                         | ✅ **CSV export added** to `DataExportPage`. Format toggle (JSON/CSV) in UI. `downloadCsv` flattens table rows to CSV with proper escaping, multi-table exports separated by `# tableName` headers. |
| 11  | Medium   | Test setup globally seeds Administrator, masking permission regressions             | ✅ **Default changed to unauthenticated.** `setup.ts` now sets `currentUser: null`. Tests that relied on the global seed (`documentMetadataEncryption`, `app`) updated to set auth explicitly. |
