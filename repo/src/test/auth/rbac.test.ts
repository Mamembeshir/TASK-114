...
describe('hasPermission — Administrator', () => {
  ...
  it('can manage users', () => expect(hasPermission(r, Permission.manageUsers)).toBe(true))
  it('can manage system', () => expect(hasPermission(r, Permission.manageSystem)).toBe(true))
  it('can view audit log', () => expect(hasPermission(r, Permission.viewAuditLog)).toBe(true))
  it('can create auctions', () => expect(hasPermission(r, Permission.createAuction)).toBe(true))
  it('can place bids', () => expect(hasPermission(r, Permission.placeBid)).toBe(true))
  it('can manage wallets', () => expect(hasPermission(r, Permission.manageWallets)).toBe(true))
  it('can approve publications', () =>
    expect(hasPermission(r, Permission.approvePublication)).toBe(true))
  it('can approve documents', () => expect(hasPermission(r, Permission.approveDocument)).toBe(true))
  it('can approve destruction', () =>
    expect(hasPermission(r, Permission.approveDestruction)).toBe(true))
  it('can manage messages', () => expect(hasPermission(r, Permission.manageMessages)).toBe(true))
})
...
describe('hasPermission — ContentEditor', () => {
  ...
  it('can create documents', () => expect(hasPermission(r, Permission.createDocument)).toBe(true))
  it('can checkout documents', () =>
    expect(hasPermission(r, Permission.checkoutDocument)).toBe(true))
  it('can view messages', () => expect(hasPermission(r, Permission.viewMessages)).toBe(true))

  // Denied
  it('cannot manage users', () => expect(hasPermission(r, Permission.manageUsers)).toBe(false))
  it('cannot place bids', () => expect(hasPermission(r, Permission.placeBid)).toBe(false))
...
describe('hasPermission — ReviewerApprover', () => {
  ...
  it('cannot place bids', () => expect(hasPermission(r, Permission.placeBid)).toBe(false))
  it('cannot manage users', () => expect(hasPermission(r, Permission.manageUsers)).toBe(false))
  it('cannot manage system', () => expect(hasPermission(r, Permission.manageSystem)).toBe(false))
...
describe('hasPermission — Participant', () => {
  ...
  it('can view auctions', () => expect(hasPermission(r, Permission.viewAuctions)).toBe(true))
  it('can place bids', () => expect(hasPermission(r, Permission.placeBid)).toBe(true))
  it('can view catalog', () => expect(hasPermission(r, Permission.viewCatalog)).toBe(true))
...
describe('requirePermission', () => {
  ...
  it('allows when user has the required permission', () => {
    ...
    expect(() => requirePermission(Permission.createAuction)).not.toThrow()
  })

  it('throws when user lacks the required permission', () => {
    useAuthStore.setState({ currentUser: makeUser(Role.Participant) })
    expect(() => requirePermission(Permission.createAuction)).toThrow(/Forbidden/)
  })
  ...
  it('Participant can placeBid but not createAuction', () => {
    useAuthStore.setState({ currentUser: makeUser(Role.Participant) })
    expect(() => requirePermission(Permission.placeBid)).not.toThrow()
    expect(() => requirePermission(Permission.createAuction)).toThrow(/Forbidden/)
  })
  ...
  it('Reviewer can approvePublication but not createPublication', () => {
    ...
    expect(() => requirePermission(Permission.approvePublication)).not.toThrow()
    expect(() => requirePermission(Permission.createPublication)).toThrow(/Forbidden/)
  })
})
