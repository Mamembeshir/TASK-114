/**
 * BootstrapAdminPage — first-run setup wizard shown when the IndexedDB
 * users table is empty (fresh production install with no demo seed).
 *
 * Creates the initial Administrator account using PBKDF2 hashing and
 * immediately logs the user in. This is the only way to create an
 * Administrator in a production build without the VITE_SEED_DEMO flag.
 *
 * Security properties:
 *  - Page is only reachable when db.users.count() === 0 (enforced in App.tsx)
 *  - Password is PBKDF2-hashed before storage — never stored in plaintext
 *  - Audit log entry written on creation
 *  - Account is NOT marked isTemporaryPassword — the operator chose the password
 */
import { useState } from 'react'
import { Eye, EyeOff, Loader2, Lock, Mail, Settings, Shield, User } from 'lucide-react'
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await db.users.add({
        ...
        isActive: true,
        // Operator chose this password intentionally — no forced change needed
      ...
      await writeAuditLog({
        ...
        entityType: 'User',
        entityId: id,
        description: `Initial Administrator account created during first-run setup: ${username}`,
      })
    ...
    <div className="min-h-screen bg-surface-950 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-8">
          ...
          <p className="text-surface-500 text-sm mt-1">First-Run Setup</p>
        </div>

        {/* ── Setup notice ──────────────────────────────────────────────── */}
        <div className="flex items-start gap-3 bg-primary-950/40 border border-primary-800/40 rounded-xl p-4 mb-6">
          <Settings className="w-4 h-4 text-primary-400 shrink-0 mt-0.5" />
          <div>
            <p className="text-sm font-medium text-primary-300">Initial Administrator Setup</p>
            <p className="text-xs text-primary-400/80 mt-0.5">
              No user accounts exist yet. Create the first Administrator account to access the portal.
              Additional staff accounts can be created from the Users page after signing in.
            </p>
          </div>
        </div>
        ...
        <div className="bg-surface-900 border border-surface-800 rounded-2xl p-8 shadow-xl">
          <h2 className="text-base font-semibold text-surface-200 mb-6">Create Administrator Account</h2>
          ...
          >
            ...
            <div className="flex items-center gap-2 py-2.5 px-3 rounded-xl bg-surface-800/60 border border-surface-700">
              ...
              <span className="text-amber-300 text-xs font-medium">Administrator</span>
            </div>
